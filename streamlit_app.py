...
# ============================================================
    ...
    # If last message is from user (e.g. from pill click), run the agent
    pending = (
        st.session_state[state_key]
        and st.session_state[state_key][-1]["role"] == "user"
    )
    if pending:
        pending_prompt = st.session_state[state_key][-1]["content"]
        with st.chat_message("assistant", avatar=agent_info["icon"]):
            with st.spinner("Agent is thinking..."):
                try:
                    response = call_agent(agent_info["fqn"], pending_prompt)
